...
function ShoppingCartView(containerNode, cartItem){
    ...
    this.drawItem = function(){
        ...
        button.onclick = function(){
            magicalStoreApp.ShopCart.removeItem(item, button);
        };
        //formating
...
function RemoveItemView(cart, button){
    console.log('remove', cart.totalPrice);
    var cartDiv = document.getElementById('cart');
    var cartItemsDiv = document.getElementById('CartItems');
    var footerDiv = document.getElementById('cartFooter');
...
/**3add button function

 * add to cart.

 * add to page

 * check if existance of cart

 * 	creat cart if no.

 * call new viewItem to cart.

 *

 * 4change add/remove button in MagicalItemView

 * update totalCost

 *

 *2 make cart function
